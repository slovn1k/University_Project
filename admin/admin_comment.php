  ...
  <div id="main">
    ...
    <div id="site_content">
      <div id="content">
		...
		<?php
			$result = mysql_query('select email, id, body from comments');
			$my_row = mysql_fetch_array($result);

			do{
				printf("<p><input type='radio' name='id' value='%s'><label>%s&nbsp;</label><label>%s&nbsp;</lable></p>",$my_row['id'],$my_row['email'],$my_row['body']);
			}
			while ($my_row = mysql_fetch_array($result));
		?>
		<input type='submit' value='Sterge_Comentariu' name='del'>
	</fomr>
      </div>
    </div>
